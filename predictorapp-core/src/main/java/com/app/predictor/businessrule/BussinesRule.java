...
public class BussinesRule {
	...
	private static final String PARAMS_FILE_PATH = "/picoplaca.xml";
	...
	public String dayEnabled(PicoPlacaForm picoPlacaForm) throws ParserConfigurationException, SAXException, IOException {
		...
		if(picoPlacaForm.getDateQuestion().get(Calendar.DAY_OF_WEEK)!= Calendar.SUNDAY && picoPlacaForm.getDateQuestion().get(Calendar.DAY_OF_WEEK)!= Calendar.SATURDAY){
			logger.info("Day : "+picoPlacaForm.getDateQuestion().get(Calendar.DAY_OF_WEEK));
			saxParser.parse(BussinesRule.class.getResourceAsStream(PARAMS_FILE_PATH), picoPlacaXmlFileParser);
		...
		{		
			logger.info("Day out: "+picoPlacaForm.getDateQuestion().get(Calendar.DAY_OF_WEEK));
